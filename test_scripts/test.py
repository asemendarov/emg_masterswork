# np.ravel(np.broadcast_to(classes, (, len(classes))).T)
...
def _y1(classes, data_len):
    return np.ravel(np.broadcast_to(classes, (data_len, len(classes))).T)


def _y2(classes, data_len):
    return np.ravel(np.meshgrid([0] * data_len, classes)[1])


def _y3(classes, data_len):
...
def main():
    ...
    X, y = [], []
    X = np.vstack(emg_data_set)
    # y = _y1(classes_list, len(emg_data_set[0]))
    # y = _y2(classes_list, len(emg_data_set[0]))
    y = _y3(classes_list, len(emg_data_set[0]))
